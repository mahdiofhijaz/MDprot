function [dist, contact_res, targetindexHeavy, notHindex, contact_per_res, contact_per] = proteinContacts(pdb, traj, target, targetChain, protChain, rCut, contactCut, allAtom)
%proteinContacts Calculates the contacts between the target and surrounding 
% residues within a certain cutoff
...
%
% * rCut is the cutoff distance in A below which a pair is considered in
% contact
%
...
for runi=1:numRuns 
    ...
    for frame = 1:Nframes(runi)
        ...
        % Grab the crd of the target/ligand and protein of this run in this frame
         ...
         % of crd_target
         [pair, dist_list] = searchrange(crd_prot, crd_target, rCut);
